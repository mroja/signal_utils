...
for k=1:c.m.N
    ...
    title(c.m.label(k),'HorizontalAlignment','center','VerticalAlignment','middle')
end
...
set(gca,'xticklabel', [], 'yticklabel', []); %turn off labeles on the plots 
set(gca,'tickdir','out');
...
for i=1:length(c.lines)
    ...
    switch c.MAX_RES
        case 1
            line([c.lines(i)-c.ref_sec(end) c.lines(i)-c.ref_sec(end)]*c.Tsize/c.map_x_size/c.dt, [0 c.maxF-c.minF],'Color',[0 0 0],'LineStyle',':')
        case 0
            line([c.lines(i) c.lines(i)]-0.5,[0  c.f_max-c.f_min]/c.df+0.5 ,'Color',[0 0 0],'LineStyle',':')
    end
