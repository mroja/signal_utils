%function [wigXY, xx, yy]=wignermap(book,f,t,Fsize,Tsize,minF,minT,dimBase,Fsamp,Xsize, Ysize)
% version 0.4 (1999-12-19, 2003-01-30)
...
wig_gabXY= sparse(wigXY);
wig_gabXY_full=zeros(Xsize,Ysize);
...
for k=1:num_atom
   ...
   if scale~=0
      if scale==dimBase % sinus
         ...
         wig_gabXY=fkron(sparse(hgab_tx), sparse(hgab_fy)); % fkron - mex 
         ...
         if NORM_XY~=0
           ...
           factor=modulus.^2./NORM_XY;
           wig_gabXY_full=full(factor*wig_gabXY);
            wigXY=wigXY+wig_gabXY_full;
         end
      end;
   else % dirac
