function [tr_data, best_lam,zz] = bc_transform(varargin)
switch nargin
case 1
        x = varargin{1};
   ...
   options = optimset('MaxFunEvals', 2000, 'Display', 'off');
    best_lam = fminsearch('bc_transform', 0, options, x, 2);
    tr_data = bc_transform(best_lam, x, 1);
case 2
    lam = varargin{1};
    x = varargin{2};
    n = length(x);
